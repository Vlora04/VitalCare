...
const MyAppointments = () => {
  ...
  return (
    <div>
      <p className='pb-3 mt-12 font-medium text-zinc-700 border-b'>Terminet e mia</p>
      <div>
        {appointments.length === 0 ? (
          <p className='mt-12 font-medium text-zinc-700 flex justify-center'>Nuk ka termine të përzgjedhura.</p>
        ) : (
          appointments.map((item, index) => (
            <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
              ...
              <div className='flex-1 text-sm text-zinc-600'>
                ...
                <p className='text-zinc-700 font-medium mt-1'>Adresa:</p>
                <p className='text-xs'>{item.docData.address.line1}</p>
                <p className='text-xs'>{item.docData.address.line2}</p>
                <p className='text-xs mt-1'><span className='text-sm text-neutral-700 font-medium'>Data dhe koha:</span> {slotDateFormat(item.slotDate)} | {item.slotTime}</p>
              </div>
